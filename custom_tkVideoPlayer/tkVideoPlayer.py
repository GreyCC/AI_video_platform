...
class TkinterVideo(tk.Label):
        ...
        while self._playing:
            ...
            if not self._paused and self._frame_number < len(self.image_sequence) - 1:
                ...
                if self.scaled or len(self._current_size) == 2:
                    self.current_img = self.original_current_img.resize(self._current_size)
                ...
                if self._frame_number % self._frame_rate/3 == 0:
                    self.event_generate("<<halfSecondChanged>>")

                if delta / 1000 >= 1 / self._frame_rate:
                    continue

                time.sleep((1 / self._frame_rate) - (delta / 1150))
                continue
    ...
    def show_commentator(self, input_path):
        img = Image.open(input_path)
        self.current_imgtk = ImageTk.PhotoImage(img.resize(self._current_size))
        self.config(image=self.current_imgtk)
