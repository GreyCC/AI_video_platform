...
class ModelEma(nn.Module):
    ...
    Keep a moving average of everything in the action_model state_dict (parameters and buffers).
    V2 of this module is simpler, it does not match params/buffers based on name but simply
    iterates in order. It works with torchscript (JIT of full action_model).
    ...
    This class is sensitive where it is initialized in the sequence of action_model init,
    ...
    def __init__(self, model, decay=0.9999, device=None):
        super().__init__()
        # make a copy of the action_model for accumulating moving average of weights
        self.ema = deepcopy(model)
        self.ema.eval()
        self.decay = decay
        self.device = device  # perform ema on different device from action_model if set
